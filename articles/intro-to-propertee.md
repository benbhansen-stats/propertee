...
For this introductory example, we will examine a single binary
...
STARplus$cond_small[STARplus$cond_at_entry == "nonexperimental"] <- NA
...
This is only for the introduction – **propertee** can keep track of
treatments with multiple levels, and includes convenience functions to
dichotomize them for outcome analyses. (This `dichotomy` functionality
is described in a separate vignette, `non-binary-treatments`.)
...
1.  [`unit_of_assignment()`](https://benbhansen-stats.github.io/propertee/reference/StudySpecificationSpecials.md):
...
2.  [`block()`](https://benbhansen-stats.github.io/propertee/reference/StudySpecificationSpecials.md):
    The identifies the variable(s) which contain block information.
    Optional.
3.  [`forcing()`](https://benbhansen-stats.github.io/propertee/reference/StudySpecificationSpecials.md):
...
Therefore, to estimate the treatment coefficient in our example, we can
run:
...
(A warning indicates that we have yet to specify either a weighting or
block fixed effect adjustment, as we need to do to make the coefficient
interpretable as an estimate of a treatment effect. We’ll turn to this
topic presently.) To estimate separate treatment coefficients for each
`race` subgroup:
