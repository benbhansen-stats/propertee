...
* Default standard errors for `teeMod` objects now incorporate methodological developments from a forthcoming paper from Wasserman and Hansen:
  ...
  2) When units used for estimating intent-to-treat effects also contribute to fitting the covariance adjustment model, the residuals from effect estimation associated with individuals in the overlapping units are replaced with residuals that use a leave-one-out estimate of the covariance adjustment model parameters.
