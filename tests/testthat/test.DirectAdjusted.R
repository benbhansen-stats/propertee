...
test_that(paste("DirectAdjusted object created correctly with weights and ",
  ...
  expect_equal(dalm$model$`(offset)`@prediction_gradient, stats::model.matrix(cmod))
...
test_that("DA ensure treatment is found", {
  ...
  dalm <- as.DirectAdjusted(lm(y ~ z, data = simdata, weights = ate(des), offset = cov_adj(cmod)))
  dalm2 <- lmitt(y ~ z, data = simdata, weights = ate(des), offset = cov_adj(cmod))
  ...
  dalm2 <- as.DirectAdjusted(lm(y ~ z, data = simdata, weights = ate(des), offset = cov_adj(cmod)))
...
test_that(paste("Conversion from lm to DirectAdjusted succeeds with weights and",
                "no SandwichLayer"), {
...
test_that(paste("Conversion from lm to DirectAdjusted succeeds with weights and",
  ...
  expect_equal(mod_da$model$`(offset)`@prediction_gradient, stats::model.matrix(cmod))
  expect_identical(mod_da$model$`(offset)`@keys, simdata[,var_names(des, "u")])
  
  mod_lmitt <- lmitt(y ~ z, data = simdata, weights = ate(des), offset = cov_adj(cmod))
...
test_that("Conversion from lm to DirectAdjusted fails without a Design object", {
  data(simdata)
...
test_that("Conversion from lm to DirectAdjusted succeeds when provided a target", {
  data(simdata)
  des <- rct_design(z ~ cluster(cid1, cid2), data = simdata)
