...
test_that("weight aliases", {
  ...
  mod1 <- lmitt(y ~ z, data = simdata, spec = spec, weights = ate())
  mod2 <- lmitt(y ~ z, data = simdata, spec = spec, weights = ett())
  mod3 <- lmitt(y ~ z, data = simdata, spec = spec, weights = att())
  ...
  mod4 <- lmitt(y ~ z, data = simdata, spec = spec, weights = "ate")
  mod5 <- lmitt(y ~ z, data = simdata, spec = spec, weights = "ett")
  mod6 <- lmitt(y ~ z, data = simdata, spec = spec, weights = "att")
