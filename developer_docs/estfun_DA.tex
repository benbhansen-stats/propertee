...
\hat{\alpha}_{0}),$$
and the $\alpha_{0}$ row of $A_{21}$ is 
...
The
$\alpha_{0}$ row of $\tilde{A}_{22}$ has
in the $\alpha_{0}$ column
...
for blocks $b \subseteq \mathcal{Q}$ and coordinates $t=2,
\ldots, k+1$ of the $k+1$ vector $\acute{\psi}$, \eqref{eq:31} gives
that
\begin{align*}
  \sum_{i \in
  \bigcup b}\frac{\partial}{\partial \alpha_{0}}\acute{\psi}_{i t} &=
...
the $\alpha_{0}$ row of $A_{21}$ and the $\alpha_{0}$ column of
...
\texttt{estfun\_DB\_blockabsorb(DA)} = $\AbsorbInterceptsEF{}
  ...
  \begin{equation*}
  \left[n^{-1}\sum_{i: b_{i}=s}\owt[{[0]}] (1-\pi_{i})\right](\alpha_{0\mathcal{D}s} - \alpha_{0\mathcal{D}})
  \end{equation*}
 ...
 \alpha_{0\mathcal{D}s}=
  \frac{\sum_{i \in s}\owt[{[0]}](1-\pi_{i})[y_{i}[0] -
    g(\vec{x}_{i}\beta)]}{\sum_{i \in s}\owt[{[0]}](1-\pi_{i})}
  \text{ and }
  \alpha_{0\mathcal{D}} = \frac{\sum_{s}p_{s}\big[\sum_{i \in
      s}\owt[{[0]}](1-\pi_{i})\big]
    \alpha_{0\mathcal{D}s}}{\sum_{s}p_{s}\big[\sum_{i \in s}\owt[{[0]}](1-\pi_{i})\big]};
  \end{equation*}
  and to obtain $\hat{A}_{\alpha_{0}\tau\,\mathbf{p}}$ we plug into our
  ${A}_{\alpha_{0}\tau\,\mathbf{p}}$-expression $z_{i}$'s for
  $\pi_{i}$s, ``$b_{i}$'' denoting the block within $\mathcal{Q}$
  that contains $i$ or $i$'s cluster $[i]$, so that  $\hat{\alpha}_{0 r}=
