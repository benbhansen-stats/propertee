...
\hat{p}_{jb} \owt[] & z_i\neq j \text{ and }0 < \sum_{i
  \in s} \owt[] \indicator{z_i=j} < \sum_{i \in \bigcup b} \owt[]\\
0 &  \sum_{i \in \bigcup b} \owt[] \indicator{z_i=j} = 0 \text{ or }
  \sum_{i \in \bigcup b} \owt[].
\end{cases}
\end{equation}
(These resemble overlap weights, $z_{i}(1-p_{jb}) +
(1-z_{i})p_{jb}$ for $i\in \bigcup b$ with $z$ binary. They coincide with
...
that contains (the cluster containing) $i$.  (Treatment being
binary, I have simplified ``$p_{0b}$'' and ``$p_{1b}$'' to
``$(1-p_{b})$'' and ``$p_{b}$'' respectively.)
...
$\hat{p}_{b_{i}}w_{\text{BFE}i}(1-z_{i})(y - g(\vec{x}\hat{\beta}) -
\hat{\alpha}_{0})$, and the $\alpha_{0}$ row of $A_{21}$ is 
\begin{equation*}
  -n_{\mathcal{Q}}^{-1}\sum_{i\in \bigcup\mathcal{Q} }
  \hat{p}_{b_{i}}w_{\text{BFE}i} (1-z_{i})g'(\vec{x}_{i}\hat{\beta}) \vec{x}_{i}.
\end{equation*}
The
$\alpha_{0}$ row of $\tilde{A}_{22}$ has $-n^{-1}\sum_{i \in \cup \mathcal{Q}}
p_{b_{i}}(1-\hat{p}_{b_{i}})w_{\text{BFE}i}$ in the
...
entails that for blocks $s \subseteq \mathcal{Q}$ and coordinates $t=2,
...
\texttt{estfun\_DB\_blockabsorb(DA)} = $\AbsorbInterceptsEF{}
 ...
 \alpha_{0\mathcal{D}s}=
  ...
  ${A}_{\alpha_{0}\tau\,\mathbf{p}}$-expression $\hat{p}_{b_{i}}$s for
  $\pi_{i}$s, ``$b_{i}$'' denoting the block within $\mathcal{Q}$
  that contains $i$ or $i$'s cluster $[i]$, as well as  $\hat{\alpha}_{0 r}=
