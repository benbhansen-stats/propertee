...
approximated by the average of observables obtained by plugging
in\footnote{%
In contrast, should 
...
\mathbf{p}) \mid \mathcal{D}]=0$, rather than substituting
$\hat{p}_{b_{i}}$ as empirically defined via $\sum_{i} \absorbInterceptsEF(\vec{v}_{i};
\hat{\mathbf{p}})=0$.}
estimates $\hat\beta$, $\hat{\tilde{\alpha}}$, $\hat{\alpha}$ and
$\hat{\tau}$ as necessary.  The sample observation $i$ contribution $\nabla_{\mathbf{p}}\acute{\psi}(\vec{v}_{i};
...
contribute to $\acute{\psi}_{i1}$). For $\hat{\tilde{A}}_{22}$ we use
the \texttt{StudySpecification} to infer the values of 
$p_{b_{i}}$ solving $\sum_{i} \EE [\absorbInterceptsEF(\vec{V}_{i};
\mathbf{p}) \mid \mathcal{D}]=0$, rather than substituting
$\hat{p}_{b_{i}}$ as empirically defined via  $\sum_{i} \absorbInterceptsEF(\vec{v}_{i};
\hat{\mathbf{p}})=0$.  In both observational studies
as well as RCTs, the expectations in this expression are evaluated by combining
\texttt{StudySpecification} information with an assumption
of block random assignment of clusters. (This circumvents the problem
of these $\hat{p}_{b}$'s not being consistently estimated under
asymptotics in which block sizes stay fixed.)
The $\tilde{\alpha}_{0}$
