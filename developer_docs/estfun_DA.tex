...
design-based perspective.
  ...
  In general
  $\EE [\nabla_{\beta}\psi(\vec{V}_{i}; \beta, \alpha_{0},\tau ) \mid
  \mathcal{M}] \equiv \nabla_{\beta}\psi(\vec{V}_{i}; \beta,
  \alpha_{0}, \tau )$; for many types of covariance model one also has
  $\EE [\nabla_{\beta}\phi(\vec{V}_{i}; \beta ) \mid \mathcal{M}]
  \equiv \nabla_{\beta}\phi(\vec{V}_{i}; \beta )$. As a result, under
  the model-based perspective $A_{11}(\beta)$ and perhaps
  $A_{21}(\beta)$ are essentially observed, i.e.
  $\hat{A}_{11}(\beta) \equiv A_{11}(\beta)$ and (perhaps)
  $\hat{A}_{21}(\beta) \equiv A_{21}(\beta)$.  These equivalences
  don't hold under the design-based perspective, and when treatment
  propensities are known (as in an RCT) it may be possible to
  calculate $A_{11}(\beta)$ and $A_{21}(\beta)$ rather than settling
  for estimates of them; but in design-based inference generally, with
  or without a known propensity, $A_{21}(\beta)$ and
  $\hat{A}_{21}(\beta)$ can be made to serve as estimates of
  $A_{11}(\beta)$ and $A_{21}(\beta)$.
...
\tilde{A}_{11}(\cdot)$.  In design based formulations these
equivalence relations become approximate equalities.)
...
In the design-based formulation such equality would hold only approximately, as in that setting
