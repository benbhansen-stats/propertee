...
even if the specification specification isn't, provided that the outcome
model is well-specified. The role of the study specification is to provide
...
An alternative would be to offer sandwich standard errors under a specification-based
...
consistent; taking the specification perspective, BH and MF have preliminary
...
\section*{StudySpecification-based SEs for differences of Hajek estimators}
\subsection*{A one-stratum special case: Neyman's insight}

In one special case a specification-based SE for \eqref{eq:hajekdiff} has been available for some time, with an optional interpretation in terms of M-estimation emerging more recently.  Suppose:
\begin{itemize}
...
the specification-based variance (conditional variance given \(\mathcal{Z}\))
of \eqref{eq:hajekdiff}. If on the other hand \(s_{uv} \leq s_{v}s_{u}\), then the expected value of this ordinary
unpooled variance exceeds \eqref{eq:hajekdiff}'s specification-based variance
...
specification-based interpretations provided that one uses the HC2
...
\subsection*{Impediments to application of the Neyman insight to
  experiment specifications beyond the 1-stratum special case}

The use of the Neyman insight to get specification-based SEs through
...
\item \label{it:moderators-multiple-outcomes}  It remains to be
  articulated how the Neyman insight can be made to furnish specification-based SEs
...
\item \label{it:prob-multreps}
  ...
  each stratum. When this doesn't hold, as in matched specifications, it's
...
\item \label{it:prob-covars}
  ...
  specification-based SE? Under what conditions? I suspect the answer is either
  no, or yes but only under restrictive conditions.\footnote{Samii and
    ...
    specification-based interpretation, they would have said just that.}
...
\item \label{it:prob-differentstatuses}
  ...
  sandwich-type variance estimates such as HC2 can't admit a specification-based interpretation as well as
...
Under the specification-based interpretation, variances and covariances of
...
notes I begin fleshing this out into methods of estimating specification-based second
...
\begin{quote}
\textit{Code specification notes:}\\
...
variances and presumed correlation of 1, we should consider software specification that doesn't tempt the user into
...
3. Current literature \citet[e.g.,][\ldots]{mukerjeeDasguptaRubin18,pashleymiratrix18varestimationblockedspecifications} may well present other specification-based covariance
estimators worth implementing as alternative B-matrix estimators.
It'd be best to specification in such a way as to facilitate alternating
...
would indeed be interpretable as specification-based SEs.%
}.
...
making no contribution. (The specification-based interpretation has no bearing on
calculation of \(A\) matrices.)

\begin{quote}
  \textit{Code specification note:}
  When the final comparison of Hajek estimates builds upon a chain of
  earlier estimates, specification-based standard errors require information
  ...
  the experimental sample, then from the specification-based perspective it
...
%%% Local Variables:
%%% mode: latex
%%% TeX-master: "specification_based_SEs_"
%%% End:
-->