...
to estimate a model. It is strongly recommended to recode the variable into
`numeric` or `character`, or you can use
[`dichotomy`](#dichotomzing-a-non-binary-treatment) (see below).

Note that weights (`ate()` or `ett()`, or `weights="ate"` or `weights ="ett"`)
only support binary treatment variables and will error on anything except binary
`numeric` or `logical`.

### Dichotomzing a Non-binary Treatment

The [`Design`](../reference/Design_objects.html) creation functions and
`ett()`/`ate()` support a `dichotomy=` argument. A `dichotomy` is presented as a
...
We place the dichotomy directly in a `Design` object:
...
summary(des1)
dichotomy(des1) <- dose >= 300 ~ dose <= 100
summary(des1)
```

You can also place the `dichotomy` in the `_design()` call direcly:

```{r}
des2 <- rct_design(dose ~ uoa(uoa1, uoa2), data = simdata,
                   dichotomy = dose >= 300 ~ dose <= 100)
summary(des2)
```

Alternatively, a `dichotomy` can be passed to `ate()`/`ett()` functions. If the
`Design` already has a `dichotomy`, it will be used, but passing an additional
`dichotomy` to the weight functions replaces the `Design`'s `dichotomy`.
