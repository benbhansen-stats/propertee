...
`modelA` say, to a covariance sample, and ending with a model along the
lines of `lm(promotion ~ treat, data=qe_spl, weights=ate(des),
offset=cov_adj(modelA))` or `lm(promotion ~ treat*demographics,
data=qe_spl, weights=ate(des), offset=cov_adj(modelA))` --- a *direct
...
treat + cov_adj(modelA), data=qe_spl, weights=ate(des))` or
`lm(promotion ~ treat * cov_adj(modelA), <...>)`. But for now our focus
...
As a result, in the model frame stored with model B, the column of predictions deriving from model A has type {`Sandwich`/`Vegan`}`LayerWithPreds`, and carries the needed info from fitting of model A to calculated sandwich SEs for the multi-decker sandwich.  Two more thing will help:

- `as.multi_decker_lm()`, a function converting accepting an object `x` inheriting from `lm`,  identifying {`Sandwich`/`Vegan`}`LayerWithPreds` objects among its predictor variables, assembling the associated {`Sandwich`/`Vegan`}`Layer` objects into a list that's tacked on to `x`.  The class attribute ot `x` is then prepended with `multi_decker_lm`, and the result is returned.
...
1\. When there's a cluster/elements distinction, `modelA` will likely be aware of elements not clusters. That's natural for the direct adjustment model also.  We can make this an expectation: when we get to calculating covariances, we'll use the design object to identify element-cluster containment associations.

2\. Rather than passing $\{\phi(\tilde{\mathbf{Y}}_i; \hat{\alpha}): i \in \mathcal{C}\}$ forward along with `modelA`, it would be preferable to pass forward what's needed to reconstruct $\{\phi(\tilde{\mathbf{Y}}_i; \hat{\alpha}): i \in \mathcal{C} \cap \mathcal{Q}\}$ from the combination of a person-record free summary of `modelA` with microdata on elements within clusters in $\mathcal{Q}$.  I propose that we serve these needs with a function that applies to fitted covariance models (or reductions of them that we might hide away and pass around as attributes of other things) and returns a function.  This returned function itself accepts a data frame as its argument, e.g. the quasiexperimental sample, and returns $\{\phi(\tilde{\mathbf{Y}}_i; \hat{\alpha}): i \in \mathcal{Q}\}$, the empirical estimating function of the covariance model, as returned by `sandwich::estfun()`, except evaluated over $\mathcal{Q}$, not whatever sample the covariance model was fit to.

3\. This leaves, $\nabla_{\alpha} \{\psi(\tilde{\mathbf{Y}}_i, \hat{\tau}, {\alpha}): i \in \mathcal{Q}\} \vert_{\alpha=\hat\alpha}$.  For $\psi()$'s that use only "predictions" of the covariance model, as ours does, the first derivative of the covariance model predictions as applied to data in $\mathcal{Q}$ will provide sufficient information from the covariance model to complete this calculation. Specifically, the derivative of whatever's returned by `optmatch::scores(modelA)`, or `predict(modelA, newdata=Q)`.  Our options include:
...
b.  Expect a fast predict method, and use it to figure numerical derivatives of the prediction.
