...
As students were blocked into schools, and then randomized within school, we establish the design based on those stratifications.
```{r}
STAR_school <- group_by(STAR_pre, schoolidk) %>% summarize(school_n = n(), school_n1 = sum(treatment), school_n0 = school_n - school_n1)
...
## STAR_design <- RCT_Design(I(1 * treatment) ~ cluster(studentid) + strata(schoolidk), data = STAR_pre)
...
ett_read_lm <- lm(read ~ treatment + schoolidk, 
...
ett_read_ethnicity_lm <- lm(read ~ treatment * ethnicity + schoolidk,
                               data = STAR_pre_post_k,
                               offset = read_y0_hat)
...
ett_read_lm <- lm(read ~ treatment, weights = cov_adj(STAR_ett, covariance_y0_read))
```
...
```{r}
## overall ATE
ate_read_lm <- lm(read ~ treatment + schoolidk, 
                  data = STAR_pre_post_k, 
                  offset = read_y0_hat)
coef(ate_read_lm)[2]
...
ate_read_ethnicity_lm <- lm(read ~ treatment * ethnicity + schoolidk, 
                            data = STAR_pre_post_k,
                            offset = read_y0_hat)
...
## notice the formula does not mention the blocking factor and no need for the offset
ate_read_lm <- lm(read ~ treatment, weights = cov_adj(STAR_ate, covariance_y0_read))
```
