...
Suppose we have models of one or both of the potential outcomes themselves, $y_{i1} = f(x_i; \beta_1)$ and $y_{i0} = g(x_i ; \beta_0)$. After fitting parameters $\hat \beta_1$ and $\hat \beta_0$, let $\hat y_{i1} = f(x_i ; \hat \beta_1)$and $\hat y_{i0} = g(x_i ; \hat \beta_0)$. For each model, we can add and subtract the estimated potential outcomes without changing the average treatment effect:

$$\Delta = \frac{1}{n} \sum_{i=1}^n y_{i1} - y_{i0} + \hat y_{i1} - \hat y_{i1} + \hat y_{i0} - \hat y_{i0} = \frac{1}{n} \sum_{i=1}^n (y_{i1} - \hat y_{i1} - \hat y_{i0})  - \frac{1}{n} \sum_{i=1}^n (y_{i0} - \hat y_{i1} - \hat y_{i0})$$

To estimate $\Delta$ we introduce a difference of Hajek estimators,

\[
\hat \Delta(w) =\frac{\sum_{i=1} w_i Z_i (Y_i - \hat y_{i1} - \hat y_{i0})}{\sum_{i=1}^n w_i Z_i} - 
\frac{\sum_{i=1} w_i (1 - Z_i) (Y_i - \hat y_{i1} - \hat y_{i0})}{\sum_{i=1}^n w_i (1 - Z_i)},
\]
where $w_i = Z_i/E(Z_i) + (1 - Z)/E(1 - Z_i)$ in the most general case, but can also include case weight information in the case of super population models and subgroup indicators for subgroup specific ATEs.
...
\[
\begin{aligned}
\Delta &= \frac{1}{n} \sum_{i=1}^n Z_i\left[(y_{i1} - \hat y_{i1} - \hat y_{i0})  - (y_{i0} - \hat y_{i1} - \hat y_{i0})\right] + \\
 & \quad  \frac{1}{n} \sum_{i=1}^n (1 - Z_i) \left[(y_{i1} - \hat y_{i1} - \hat y_{i0})  - (y_{i0} - \hat y_{i1} - \hat y_{i0})\right]  \\
 &= \frac{n_1}{n} \delta(Z) + \frac{n_0}{n} \delta(1 - Z),
\end{aligned}
\]
where $n_1 = \sum_{i=1}^n Z_i$ and $n_0 = \sum{i=1}^n 1 - Z_i$. To estimate $\delta(Z)$, we must estimate $y_{i0}$ for the treated group, typically by substituting $\hat y_{i0}$. Consequently, the estimator reduces so
$$\hat \delta(Z, w) = \frac{\sum_{i=1}^n w_i Z_i\left[(Y - \hat y_{i1} - \hat y_{i0}) - (\hat y_{i0} - \hat y_{i1} - \hat y_{i0})\right]}{\sum_{i=1}^n w_i Z_i} = \frac{\sum_{i=1}^n w_i Z_i(Y - \hat y_{i0}) }{\sum_{i=1}^n w_i Z_i}$$
Likewise for $\hat \delta(1 - Z)$.
...
(ett_read_manual <- filter(STAR_pre_post_k, treatment) %>% 
    summarize(mean(read - read_y0_hat, na.rm = TRUE)))
...
(ett_read_afam_manual <- filter(STAR_pre_post_k, treatment) %>% 
    ...
    summarize(mean(read - read_y0_hat, na.rm = TRUE)))
```
I am somewhat confused by these results. The benchmark calculations for the subgroup ETTs are very different than the overall ATEs estimated elsewhere, but they do match what we would estimate for the conditional mean of $y_1 \mid x, Z = 1$. Whereas the model coefficients seems very plausible.
...
```{r}
# benchmark
tmp <- group_by(STAR_pre_post_k, treatment) %>%
  summarize(mean(read - read_y1_hat - read_y0_hat, na.rm = TRUE)) 
diff(tmp[, 2, drop = TRUE])

ate_read_lm <- lm(read ~ treatment, data = STAR_pre_post_k, offset = read_y1_hat + read_y0_hat)
coef(ate_read_lm)[2]

group_by(STAR_pre_post_k, treatment, ethnicity) %>%
  summarize(v = mean(read - read_y1_hat - read_y0_hat, na.rm = TRUE)) %>%
  pivot_wider(id_cols = ethnicity, values_from = v, names_from = treatment) %>%
  mutate(ate = `TRUE` - `FALSE`)
    
ate_read_ethnicity_lm <- lm(read ~ treatment * ethnicity, 
                            data = STAR_pre_post_k,
                            offset = read_y1_hat + read_y0_hat)
