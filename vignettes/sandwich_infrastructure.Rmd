...
will have a column `(weights)` of class `WeightedDesign`, and a column
`offset(cov_adj(cmod))` that's either of class `SandwichLayer`, if
`cov_adj()` was able to arrange this, or `PreSandwichLayer` as a
fallback.
...
$A_{21}$;
4. Estimates of the direct adjustment model's A "bread matrix" $(n A_{22})^{-1} =
\{\nabla_\tau \sum_{i \in \mathcal{Q}}
\mathbb{E}[\psi(\tilde{\mathbf{Y}}_i, \tau, \hat{\alpha})]
\vert_{\tau=\hat\tau}\}^{-1}$, i.e. the inverse of its unscaled Fisher information
w.r.t. $\tau$ only, along with the (similarly unscaled) "meat" or B matrix $n B_{22} =
\mathrm{Cov}[\sum_{i \in \mathcal{Q}} \psi(\tilde{\mathbf{Y}}_i,
{\tau}, {\alpha})]_{({\tau}, {\alpha})=(\hat{\tau}, \hat{\alpha})}$;
5. The covariance model's bread matrix $(n \hat{A}_{11})^{-1} = \{\sum_{i \in \mathcal{C}}
...
In (5), observed information $\sum_{i \in \mathcal{C}}
\nabla_\alpha [\phi(\tilde{\mathbf{Y}}_i;
