...
makeDataShapes <- function(n, shape, tdist=FALSE, tau=0, plt=FALSE){
  ...
  yc <- yc+if(tdist) stats::rt(n,3) else stats::rnorm(n)
...
flexPoly <- function(dat,deg){
  ...
  covForm <- as.formula(parse(text = paste0("Y~Z+poly(R,", deg, ",raw=TRUE)"))[[1L]])
  ### until then
  # covForm <- "Y~R+Z"
  # if(deg>1)
...
polySim <- function(n, degs, shape, tdist, tau){
  ...
  Reduce(c, lapply(degs,func))
}

totalPolySim <- function(nreps, n, degs, tau, cluster = NULL) {
  ...
  for(shape in c("lin", "antiSym", "wass")) {
    for (tdist in c(TRUE)) {
      res[[paste0(shape,'_',ifelse(tdist, "t", "norm"))]] <-
        appFunc(seq_len(nreps), function(i) polySim(n,degs,shape,tdist,tau))
    }
  }
  res
}
