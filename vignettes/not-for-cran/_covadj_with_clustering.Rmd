...
`propertee`'s suite of internal functions for computing these elements block matrices follow
...
### Validating the `propertee` Variance Estimates
This section aims to validate `propertee`'s machinery for computing $\text{Cov}(\hat{\tau})$.
...
#### Verifying `propertee`'s Block Matrix Outputs with Expected Outputs
We derived the forms of the covariance-adjusted sandwich variance estimate matrix
components previously, so we verify the outputs of `propertee`'s internal functions
match those:
...
#### Verifying `propertee`'s Block Matrix Outputs to the `sandwich` Package
...
#### Verifying `propertee`'s Variance Estimate to `geex`
...
`propertee`'s variance estimates; we just need to define the estimating functions
...
test_that("compare with geex", {
  expect_equal(vcovDA(damod, type = "CR0", cadjust = FALSE)[2, 2],
               geexRes@vcov[4, 4])
})
```

#### Heuristic Checks
We now make heuristic checks of `propertee`'s covariance-adjusted variance
...
As a result, `propertee`'s variance estimate for the treatment effect and the
...
test_that("treatment effect variance unchanged by regression adjustment", {
  ...
  expect_equal(
    ...
    vcovDA(damod, cadjust = FALSE, type = "CR0")
  )

  expect_equal(
    vcovDA(damod, cadjust = FALSE, type = "CR0")[2, 2],
...
```{r propertee_better, echo = TRUE, eval = TRUE}
test_that("propertee has smaller variance estimate than big model", {
...
$A_{21}$, and $B_{22}$ in `propertee` (and `geex`'s) variance
calculation:
...
`propertee`'s estimated variance of the treatment effect estimate should be larger
