...
```{r}
ate(des2, data = simdata, dichotomize = dose < 200 ~ dose >= 200)
ate(des2, data = simdata, dichotomize = dose > 200 | dose < 100 ~ .)
ate(des2, data = simdata, dichotomize = . ~ dose %in% c(50, 300))
ate(des2, data = simdata, dichotomize = dose <= 100 ~ dose == 300)
```

Alternatively, we could place the dichotimization in the Design instead.

```{r}
des2b <- dichotomize_design(des2, dose > 200 ~ dose <= 100)
ate(des2b, data = simdata)
des3 <- rct_design(dose ~ cluster(cid1, cid2), data = simdata, dichotomize = dose > 200 ~ dose <= 100)
