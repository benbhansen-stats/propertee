...
\newcommand{\owt}[1][z_i]{\ensuremath{\check{w}_{i[#1]}}}
\usepackage{natbib}
\usepackage{bibentry}
...
\item $\owt[j]$ represents the product of $w_{i}$, a
  frequency or importance weight
  attaching to observation unit $i$ (or 1 if no such weight was
  given) and, as requested by the user,
  odds- or inverse probability-based weights attaching to $i$'s
...
\texttt{lm()}'s default treatment effect parameterizations, whereas
\eqref{eq:9} in \S~\ref{sec:accomm-interc-via} below presents
another $\psi()$ adding block-specific intercepts to \eqref{eq:14}.)
I've written ${\Psi}(\cdot)$ as
...
conditional on
\[ \mathcal{M} = \sigma\left(\left[(X_{i}, W_{i}, Z_{i}: i \in c): i \in
...
&\left(
           ...
           \big[y_{i}[z_{i}]
                                               -
                                               g(\vec{x}_{i}\beta)-\tau_{z_{i}}\indicator{z_{i}>0}\big](
                                               \indicator{z_{i}=1} - \sum_{r=1}^{s}p_{1r}\indicator{b_{i}=r})\\
                  \vdots \\
{}           \big[y_{i}[z_{i}]
                                               - g(\vec{x}_{i}\beta) -
                                               \tau_{z_{i}}\indicator{z_{i}>0}\big](\indicator{z_{i}=k}
...
$0 = \sum_{i}\grave{\phi}(\tilde{\mathbf{y}}_{i}; \mathbf{p})$ have
covariance 0.  Writing $\Upsilon =
[\upsilon(\tilde{\mathbf{y}}_{i}; \mathbf{p}, \beta, \tau) : i]'$, one
obtains valid\footnote{%
The residuals contributing to $\Upsilon$ via \eqref{eq:upsilondef} do not remove stratum
...
basis for variance-covariance estimation.}
  sandwich estimates by substitution of $\Upsilon$ for
...
In design-based formulation, randomness of $Z$ can propagate to
sampling variability of $\hat{\mathbf{p}}$,  so we must attend to
variance contributions from $[\grave{\phi}(z_{i}, \mathbf{p}): i]$. 
...
accounting in covariance estimation, since both $Z$ and the moderating
variable are held fixed by conditioning. We get a legit covariance
without doing anything special, using an $\Upsilon$ given by
\begin{multline*}
  \upsilon(\tilde{\mathbf{y}}; \tau, \mathbf{p}, \beta) =
    \indicator{i \in \bigcup \mathcal{Q}}\owt{} \times \\
\left(
...
$\phi(\tilde{\mathbf{Y}}_{i}; \beta)$ in this way then \eqref{eq:19}
