...
Observe that if we were to replace ``$n_{\mathcal{C}}$'' and ``$n$''
with arbitrary nonzero constants, these expressions would define the
same $\tilde{\beta}_{(n)}$ and $\tilde{\tau}_{(n)}$. 
Taking $n_{\mathcal{C}}$ and $n$ to be the numbers of elements (not
clusters) in the covariance and in the combined samples, respectively,
promotes alignment with existing \texttt{sandwich} functions%
\footnote{%
  This $n_{\mathcal{C}}$ is the constant that \texttt{sandwich}
  functions will select for the covariance model, given its sample
...
$n$.)  A potential disadvantage is that it makes $A_{21}$ and
...
design-based perspective.
  ...
  \subsubsection{Conceptual vs practical bread estimates}
...
$A_{21}(\beta)$ as given in \eqref{eq:13}. 
  ...
  \paragraph{$\tilde{A}_{21}$ and $\tilde{A}_{22}$, rescalings of
    ${A}_{21}$/${A}_{22}$ for numerical stability}
  In situations with $n_{\mathcal{C}} \gg n_{\mathcal{Q}}$, scaling
  ${A}_{21}$ and ${A}_{22}$ by $n^{-1}$, as called for above, may
  invite numerical instability; $n_{\mathcal{Q}}$ seems more
  appropriate. Write $\tilde{A}_{21} = (n/n_{\mathcal{Q}}) {A}_{21}$ and
  $\tilde{A}_{22} = (n/n_{\mathcal{Q}}){A}_{22}$, i.e.
  \begin{align*}
    \tilde{A}_{21}(\beta) &= n_{\mathcal{Q}}^{-1}\sum_{j\in \bigcup
        \mathcal{Q}}\EE[ \nabla_{\beta}\psi(\vec{Z}_{j};
        \tau, \alpha, \beta )] \\
    \tilde{A}_{22}(\alpha, \tau) &= n_{\mathcal{Q}}^{-1}\sum_{j\in \bigcup
        \mathcal{Q}}\EE[ \nabla_{\alpha, \tau}\psi(\vec{Z}_{j};
        \tau, \alpha, \beta )].
  \end{align*}
  Due to mutual cancelation of $n/n_{\mathcal{Q}}$ and
  $(n/n_{\mathcal{Q}})^{-1}$ factors, \eqref{eq:6}
  continues to hold with $\tilde{A}_{21}$ and $\tilde{A}_{22}$
  substituted for ${A}_{21}$ and ${A}_{22}$.
...
\begin{multline}
  ...
  M_{22} -
                                 \frac{n^{1/2}}{n_{\mathcal{C}}^{1/2}}[M_{21}A_{11}^{-t}A_{21}^t
                                 + A_{21}A_{11}^{-1}M_{12}] +
                                 \frac{n}{n_C}A_{21}A_{11}^{-1}M_{11}A_{11}^{-t}A_{21}^{t}, \label{eq:12}
                               \end{multline}
