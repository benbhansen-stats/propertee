...
$\psi(\cdot)$ as in \eqref{eq:5} above, has the same solutions as $\sum_{i}\psi(\vec{z}_{i};
  \alpha, \tau, \beta)=0$, with $\alpha=\rho_{0}$, $\tau_{j}=\rho_{j}-\alpha$ and $\psi(\cdot)$ now given by, writing $\owt[] \defeq \owt{}$, 
...
Recall that $\tau_{0} = \rho_{0}-\rho_{0}=0$.  If \texttt{absorb=FALSE}, then we revert to \eqref{eq:14} with ``$\alpha_0$'' for ``$\alpha$,'' i.e.
\begin{equation*}
         \psi(\vec{z}_{i};
