...
\end{align}
Note that this defines an intercept parameter $\alpha_{0}$ that does not arise
in the canonical reference implementation, which has $\alpha_{1},
\ldots, \alpha_{s}$ but not $\alpha_{0}$. It averages control group
observations within each block, with weights proportional to
$\owt[0]$, before averaging these blockwise averages using weights
proportional to $\sum_{i\in b}\owt \hat{p}_{0b}(1-\hat{p}_{0b})$. In
the two-group case, effect estimates $\hat{\tau}$ coincide with
averages of stratumwise differences,
\begin{equation*}
  \{\sum_{i \in b}
z_{i}w_{i}[y_{i}[1] - g(\vec{x}_{i}\beta)]\}/(\sum_{i \in b}
z_{i}w_{i}) - \{\sum_{i \in b}
(1-z_{i})w_{i}[y_{i}[0] - g(\vec{x}_{i}\beta)]\}/(\sum_{i \in b}
(1-z_{i})w_{i}),
\end{equation*}
also weighting contribution blocks $b$ in proportion with $\sum_{i\in
  (\bigcup b)}\owt
\hat{p}_{0b}(1-\hat{p}_{0b})$.

By simplifying sums of \eqref{eq:upsilondef},  block $b$'s contribution
$\sum_{i \in (\bigcup b)} \acute{\psi}(\vec{v}_{i}; \alpha_{0}, \tau,
\mathbf{p}, \beta)$ can be seen to equal
\begin{multline}
  \big(\sum_{i} \owt[]\big)
        \left(
          \begin{array}{c}
            (1-{p}_{0b})
            \hat{p}_{0b}\left(\frac{\sum_{{z_i=0}}\owt[]\tilde{y}_{i}}{\sum_{{z_i=0}}\owt[]}
            \right)\\
         {} (1-{p}_{1b})
...
$y_{i}[z_{i}] - g(\vec{x}_{i}\beta)-\alpha_{0}-\tau_{z_{i}}$.
...
design;  $\hat{\alpha}_0$ will correspond to an $\owt[]$-weighted
control group mean.)
    ...
    \breve{\psi}(\vec{v}_{b}; \theta, \mathbf{p}) = \Big(\sum_{i} \owt[]\Big) \times\\
        \left(
          \begin{array}{c}
            ...
            \hat{p}_{0b}\left[\frac{\sum_{{z_i=0}}\owt[]\tilde{y}_{i}}{\sum_{{z_i=0}}\owt[]}
            -
    \hat{\alpha}(\mathbf{v}_{b}; \theta)\right]\\
         {} (1-{p}_{1b})
...
\eqref{eq:28}.  (To implement this design-based estimator, we can
  ...
  \label{eq:18}
    ...
    \indicator{i \in \bigcup \mathcal{Q}} \owt[]
     \left(
       \begin{array}{c}
         % (
         % \indicator{z_{i}=0}-p_{0b_{i}}-\gamma_{0}
         % \tilde{m}_{i}
         % )m_{i} \\
         (
...
&=
  ...
  &\operatorname{vec}  \left(%
                                                 \left(
                                                 \begin{array}{ccc}
                                                   % (\indicator{z_{i}=0}-p_{0b_{i}}-\tilde{m}_{1i}\gamma_{1})m_{1i}&\ldots&(\indicator{z_{i}=0}-p_{0s}-\tilde{m}_{ti}\gamma_{t})m_{ti}
                                                   % \\
