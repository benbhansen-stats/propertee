...
\subsection{$\psi(\vec{Z}_{i};
  \alpha, \tau, \beta)$, an intercept-form equivalent of $\grave{\psi}(\vec{Z}_{i};
  \rho, \beta)$}
\label{sec:psit-rho_0-tau}

Write $\tau_{j}=\rho_{j}-\rho_{0}$ and introduce the intercept parameter ``$\alpha$,''  for the time being one-dimensional with $\alpha_{0}=\rho_{0}$.   The estimating equation $\sum_{i}\grave{\psi}(\vec{z}_{i}; \rho, \beta)=0$,
$\grave{\psi}(\cdot; \cdot)$ as in \eqref{eq:5} above, has the same solutions as $\sum_{i}\psi(\vec{z}_{i};
...
\begin{equation} \label{eq:14}
         ...
         \left( \begin{aligned}
           \owt[{[a_{i}]}] &\big[y_{i}[a_{i}] - g(\vec{x}_{i}\beta)-
                  \alpha_{0} - \sum_{j=1}^{k}\tau_{j}\indicator{a_{i}=j}\big]\\
           \owt[{[1]}]\indicator{a_{i}=1} & \big[y_{i}[1] - g(\vec{x}_{i}\beta)-
                  \alpha_{0}-\tau_{1}\big]\\
                  \vdots \\
           \owt[{[k]}]\indicator{a_{i}=k} & \big[y_{i}[k] - g(\vec{x}_{i}\beta)-
                 \alpha_{0} - \tau_{k}\big]\\
                \end{aligned}\right), 
\end{equation}
the omission of a ``$\indicator{a_{i}=0}$'' factor from the topmost
expression being intentional and deliberate.
Abbreviating ``$\owt{}$'' as ``$\owt[]$'' and ``$y_{i}[a_{i}]$'' as
``$y_{i}$,'' we have
\begin{equation}\label{eq:27}
  ...
  \owt[] \Big[y_{i} - g(\vec{x}_{i}\beta)-
                  \alpha_{0} -
                  \sum_{j=1}^{k}\tau_{j}\indicator{a_{i}=j}\Big]
                  \begin{pmatrix} 1 \\ \indicator{a_{i}=1}\\ \vdots
                    \\ \indicator{a_{i}=k}\end{pmatrix} .
\end{equation}
Since the solution in 
$(\alpha_{0}, \tau_{1}, \ldots, \tau_{k})$ of
$\sum_{i}\psi(\vec{z}_{i}; \alpha, \tau, \beta) =0$ satisfies
