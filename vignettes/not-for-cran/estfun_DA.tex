...
\ldots, k$. (I say ``for now'' because there are some other
possibilities for the canonical form, noted below under ``absorption''.  We have written $\tilde{\Psi}(\cdot)$ as
...
\right)A^{-1}
\]
(recalling that $\psi(\tilde{\mathbf{Y}}_{j}; \tau, \beta) = 0$ when
$j\not\in \bigcup \mathcal{Q}$.)
Here we take sample size constants $n_{\mathcal{C}}$ and $n$ to be the number of elements
(not clusters) in the covariance and in the the combined samples,
respectively, as \texttt{sandwich} functions will want to do given the
dimensions of the estfun's of the covariance and the DirectAdjusted
objects.  There's no correct or incorrect $n$, provided that we
calculate the covariance above correctly and keep track of what we've
done.

The choice of sample size constant affects B but not A matrices,
at least as construed here (and as implemented in the sandwich
package). As it does affect the B matrix, and as we're going to
construe B matrices slightly differently here than in prior threads,
I'll use the letter M instead of B. We have
\[
...
\[
  \begin{array}{cc}
    ...
    \end{array}
\]
Note that $M_{11}$ is a natural meat matrix, while $M_{22}$ and
$M_{12}$/$M_{21}$ are progressively more weird.
...
\subsection{$\psi(\tilde{\mathbf{Y}}_{i};
  \rho, \beta)$ with intercept and $\rho$'s}

The estimating equation $\psi(\tilde{\mathbf{y}}; \rho, \beta)=0$
above has the same solutions as, with $\tau_{j}=\rho_{j}-\rho_{0}$ for
$1\leq j \leq k$, 
\begin{equation*}
%         \label{eq:5}
         0 =
...
\subsection{Accommodating intercept(s) via absorption}
As $\rho_{0}$ is a nuisance parameter, let's use the
Frisch-Waugh-Lovell (FWL) Theorem to get rid of it.  In the process we'll
  ...
  \begin{array}{cc}
    M_{11}  = n_{\mathcal{C}}^{-1}\operatorname{Cov}[\sum_{i\in \bigcup
             ...
             \mathcal{C}}\phi(\tilde{\mathbf{Y}}_{i};
                                                                  ...
                                                                  \mathcal{Q}}\psi'(\tilde{\mathbf{Y}}_{i}; \tau, \beta )]\\
...
There are random estimating functions $n_{\mathcal{C}}^{-1}\sum_{i\in\mathcal{C}}\phi(\tilde{\mathbf{Y}}_{i}; \beta)$  and 
       $n_{\mathcal{Q}}^{-1}\sum_{i\in
         \mathcal{Q}}\psi(\tilde{\mathbf{Y}}_{i}; \tau, \beta)$,
       $\psi(\tilde{\mathbf{Y}}_{i}; \tau, \beta) =
       w_{i\mathcal{Q}}[y_{i} - \tau(z_{i} - \bar{z}) -
...
\[ \begin{array}{c} 0 \\ 0 
   \end{array} = \left(
     \begin{array}{c}
       ...
       n_{\mathcal{Q}}^{-1}\sum_{i\in \mathcal{Q}}\psi(\tilde{\mathbf{y}}_{i}; \tau, \beta )
     \end{array}
\right).
\]

We seek to represent $\sum_{i\in
         \mathcal{Q}}\psi(\tilde{\mathbf{Y}}_{i}; \tau, \beta)$ as a
       linear combination of
       \begin{align}
         \label{eq:1}
         \sum_{i \in \mathcal{Q}}\psi_{0i} =& \sum_{i \in
                                             \mathcal{Q}}w_{i\mathcal{Q}}[y_{i}
                                             - g(\vec{x}_{i}\beta) -
                                             \rho_{0}](1-z_{i})\\
         \label{eq:2}
