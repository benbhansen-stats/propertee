...
\[\hat{\mu}_k[\mathbf{v}] = \frac{\sum_i w_{i[k]} [\![A_i=k]\!] v_i}{\sum_i w_{i[k]}  [\![A_i=k]\!]}\]
and their differences
\begin{equation}\label{eq:hajekdiff}
\hat{\mu}_k[\mathbf{u}] - \hat{\mu}_0[\mathbf{v}].
\end{equation}
The case weights $\{w_{i[k]}:i\leq n, k\leq K\}$ are the products of
an frequency or importance weight attaching to the cluster and, as
appopriate, an inverse probability of assignment weight; they are permitted
to differ by assigned condition.
With $w_{i[j]} =[\mathbb{P}(A_{i}=j \mid \mathcal{Z}]^{-1}$ for
$j=0, \ldots, K$,
...
$w_{i[j]} =\pi_{i}^{-1}[\mathbb{P}(A_{i}=j)]^{-1}$ for $j=0, \ldots, K$
...
$\mathrm{Var}(\sum_i w_{i[k]} [\![A_i=k]\!] \mid \mathcal{Z}) = \mathrm{Var}(\sum_i
w_{i[0]} [\![A_i=0]\!] \mid \mathcal{Z}) = 0$, but the biases are
...
\item constant weights, i.e. there exist $w_0$ and $w_k$ s.t. $w_{i[k]}= w_k$ and $w_{i[0]}= w_0$ for each $i$; and
...
$\{w_{i[0]} :i\}$ or  $\{w_{i[k]} :i\}$, to vary, as is necessary
...
(\mu_{0}, \ldots, \mu_{K}) \mapsto
  \left[
\begin{array}{c}
\sum_i [\![a_i=K]\!] w_{i[K]} (y_{Ki} - \mu_K) \\
\vdots \\
\sum_i [\![a_i=0]\!] w_{i[0]} (y_{0i} - \mu_0) 
\end{array}
\right]
.
\end{equation}
...
Neyman's two-sample unpooled variances SE to observations $\{ w_{i[k]}
(y_{ki} - \mu_k) : A_i = k\}$ and $\{ w_{i[0]} (y_{0i} - \mu_0) : A_i =
0\}$.  Impediment~\ref{it:prob-noweights} is solved.

For standard errors of the difference of Hajek estimators within a
subgroup $g$,
\begin{equation*}
 \frac{\sum_i w_{i[k]} [\![A_i=k, G_{i} =g]\!] y_{ki}}{\sum_i w_{i[k]}
     [\![A_i=k, G_{i}=g]\!]}
 -
  \frac{\sum_i w_{i[0]} [\![A_i=0, G_{i} =g]\!] y_{0i}}{\sum_i w_{i[0]}
     [\![A_i=0, G_{i}=g]\!]},
\end{equation*}
we can simply fold the subgroup indicator into the weights,
weighting by $\{\tilde{w}_{i[j]}:j; i\} = \{w_{i[j]}[\![G_{i}=g]\!]: j; i\}$ as opposed to $\{w_{i[j]}:j;
...
\begin{equation*}
  \mathrm{Cov}\left(
    \frac{\sum_i w_{i[k]} [\![A_i=k]\!] v_{ki}}{\sum_i w_{i[k]}
      [\![A_i=k]\!]} -
    \frac{\sum_i w_{i[k]} [\![A_i=k]\!] v_{0i}}{\sum_i w_{i[k]}  [\![A_i=k]\!]},
...
\begin{array}{c}
\sum_{i\in S} [\![A_i=K]\!] w_{i[K]} (y_{Ki} - \mu_K) \\
\vdots \\
\sum_{i\in S} [\![A_i=0]\!] w_{i[0]} (y_{0i} - \mu_0)
\end{array}
\right] \mid \mathcal{Z}\right), \quad S \in \mathcal{P}.
\end{equation}
For any stratum $S$ and condition $k$ s.t.  $\sum_{i \in S}  [\![A_i=k]\!] =1$, $\mathrm{Var}\left\{ \sum_{i\in S} [\![A_i=k]\!] w_{i[k]} (y_{ki} - \mu_k) \right\}$ is not estimable, just as the variance $[(\# S)-1]^{-1}\sum_{i \in S} (y_{ki} - \bar{y}_k)^2$ is not estimable. On the other hand, the second moment
\[ \mathbb{E}\left[\left\{ \sum_{i\in S} [\![A_i=k]\!] w_{i[k]} (y_{ki} - \mu_k) \right\}^2\right]
\]
\textit{is} estimable, by it sample realization \(\left\{ \sum_{i\in
    S} [\![a_i=k]\!] w_{i[k]} (y_{ki} - \mu_k) \right\}^2\): in
M-estimation, we're entitled to treat $\mu_k$ is a fixed constant, not
a random variable. Since the second moment  bounds the variance from above,  \(\{\sum_{i\in S} [\![A_i=k]\!] w_{i[k]} (y_{ki} - \mu_k)\}^2\) is a safe, conservative variance estimate.
...
corresponding to a correlation of 1 between $\sum_{i\in S} [\![A_i=k]\!] w_{i[k]}
(y_{ki} - \mu_k)$ and $\sum_{i\in S} [\![A_i=0]\!] w_{i[0]} (y_{0i} -
...
$\sum_{i\in S} [\![A_i=k]\!] w_{i[k]} (y_{ki} - \mu_k)$'s and
$\sum_{i\in S} [\![A_i=0]\!] w_{i[0]} (y_{0i} - \mu_0)$'s estimated
...
\verb|y ~ a + x| with
unit weights $\sum_{j=1}^{K}w_{i[j]}[\![a_{i}=j]\!]$ --- that is, the
