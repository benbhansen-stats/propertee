...
The data for this example were randomly simulated using the `synthpop` package in `R` based on data originally collected by [Lindo, Sanders, and Oreopoulos (2010; hereafter LSO)](https://www.aeaweb.org/articles?id=10.1257/app.2.2.95). (The original real data can be found [here](https://www.openicpsr.org/openicpsr/project/113751/version/V1/view); code to simulate the fake data can be found [here](https://github.com/benbhansen-stats/flexida/blob/main/data-raw/makeRDDdata.r).)
...
figDat <- aggregate(fdat[,c('nextGPA','lhsgrade_pct')],by=list(R=fdat$R),
                    FUN=mean,na.rm=TRUE)
figDat$n <- as.vector(table(fdat$R))
...
What characterizes discontinuity (RD) designs, including the LSO study, is that treatment is assigned as a function of a numeric "running variable" $R$, along with a prespecified cutoff value $c$, so that treatment is assigned to subjects $i$ for whom $R_i>c$, or for whom $R_i<c$. 
...
In the LSO study, $i$ indexes students, centered first-year GPA $R_i$ is the running variable, and the treatment under study is AP placement. Then $Z_i=\mathbf{1}\{R_i<0}$.
(In "fuzzy" RD designs, $R$'s value relative to $c$ doesn't completely determine treatment, but merely affects the probability of treatment, so subjects with, say, $R_i>c$ are more likely to be treated than those with $R_i<c$; in fuzzy RD designs, $Z_i=\mathbf{1}\{R_i>c\}$ is typically modeled as an instrument for treatment receipt.)

RD designs hold a privileged place in causal inference, because unlike most other observational study designs, the mechanism for treatment assignment is known. 
...
Traditionally, the typical way to analyze data from RD designs is with ANCOVA, by fitting a regression model such as
...
### The `flexida` Approach to RD Designs

The `flexida` approach to RD designs breaks the data analysis into three steps:
...
## Analyzing an RD design in `flexida`
...
This stage of the analysis is beyond the scope of this vignette, and does not require the flexida package. 
...
fdatW <- subset(fdat,abs(R)<=0.5)

fdatW$id <- 1:nrow(fdatW)
des <- RD_Design(Z~forcing(R)+unitid(id),data=fdatW)
```

### Modeling $Y_C$ as a function of $R$
We will consider two potential models $\tilde{g}(\cdot)$ of $Y_C$ as a function of $R$.
First, the standard OLS model:

```{r olsGmod}
g1 <- lm(nextGPA~R+Z,data=fdatW)
```
...
if(require(robustbase)){
  lmrob(nextGPA~poly(R,5)+Z,data=fdatW)
} else g1
```
...
mean(fdatW$nextGPA[fdatW$Z]-yhat1[fdatW$Z])-
  mean(fdatW$nextGPA[!fdatW$Z]-yhat1[!fdatW$Z])

#### method 2:
coef(lm(nextGPA~Z, offset=yhat1,data=fdatW))['ZTRUE']
...
mean(fdatW$nextGPA[fdatW$Z]-yhat2[fdatW$Z])-
  mean(fdatW$nextGPA[!fdatW$Z]-yhat2[!fdatW$Z])

#### method 2:
coef(lm(nextGPA~Z, offset=yhat2,data=fdatW))['ZTRUE']
