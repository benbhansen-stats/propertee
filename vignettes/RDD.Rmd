---
title: "Regression Discontinuity StudySpecifications"
author: "Adam Sales"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Regression Discontinuity StudySpecifications}
...
What characterizes discontinuity (RD) specifications, including the LSO study, is that treatment is assigned as a function of a numeric "running variable" $R$, along with a prespecified cutoff value $c$, so that treatment is assigned to subjects $i$ for whom $R_i>c$, or for whom $R_i<c$.
...
(In "fuzzy" RD specifications, $R$'s value relative to $c$ doesn't completely determine treatment, but merely affects the probability of treatment, so subjects with, say, $R_i>c$ are more likely to be treated than those with $R_i<c$; in fuzzy RD specifications, $Z_i=\mathbf{1}\{R_i>c\}$ is typically modeled as an instrument for treatment receipt.)

RD specifications hold a privileged place in causal inference, because unlike most other observational study specifications, the mechanism for treatment assignment is known.
...
### Analyzing RD StudySpecifications with ANCOVA

Traditionally, the typical way to analyze data from RD specifications is with ANCOVA, by fitting a regression model such as
...
### The **propertee** Approach to RD StudySpecifications

The **propertee** approach to RD specifications breaks the data analysis into three steps:
...
## Analyzing an RD specification in **propertee**

### Determining the window of analysis
There is an extensive literature on determining the appropriate window of analysis for an RD specification. See, for instance, Imbens and Kalyanaraman (2012) and Sales and Hansen (2020).
...
### Initializing the RD StudySpecification Object
...
```
Defining an RD specification requires, at minimum, identifying the running variable(s) $R$, as well as how $R$ determines treatment assignment.
In our example,
```{r rdStudySpecification}
